...
def split_transmart_data_file(input_data_file, **optional_args):
	...
	cmpt = 0
	variables_type_list = []
	transmart_data = open(input_data_file, "r")
	for line in transmart_data:
		if(cmpt == 0):
			line_in_array = line.split("\t")
			...
			for elt in line_in_array:
				variable = elt.split("\\")
				try:
					if variable[3] not in variables_type_list:
						variables_type_list.append(variable[3])
				except:
					pass
				header_file.write(str(elt)+"\n")
			header_file.close()
		cmpt += 1
	transmart_data.close()


	for type_variable in variables_type_list:

		## 

		output_file_name = str(type_variable)+".csv"
		...
		for line in data:
			...
			if(cmpt == 0):
				...
				if(specific_group_selected):
					for element in line_in_array:
						element_in_array = element.split("\\")
						if(type_variable in element_in_array):
							index_to_keep.append(index)
							header_in_string += str(element)+","
						elif("CSPHASE" in element_in_array):
							phase_index = index
